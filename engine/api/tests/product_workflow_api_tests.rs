...
#[test]
fn get_product_workflow_status_returns_steps() {
    let api = HotSasApi::new(fake_services());
    let dto = api
        .get_product_workflow_status()
        .expect("must return status");

    assert_eq!(dto.app_name, "HotSAS Studio");
    assert!(
        !dto.workflow_steps.is_empty(),
        "status must contain workflow steps"
    );
}

#[test]
fn run_product_beta_self_check_returns_checks() {
    let api = HotSasApi::new(fake_services());
    let dto = api
    ...
    assert!(
        dto.current_project.is_some(),
        "self-check should create a project"
    );
    assert!(
        !dto.module_statuses.is_empty(),
        "self-check must contain module statuses"
    );
}

#[test]
fn create_integrated_demo_project_sets_current_project() {
    let api = HotSasApi::new(fake_services());
    let dto = api
        .create_integrated_demo_project()
        .expect("must create demo project");

    assert_eq!(dto.name, "RC Low-Pass Demo");
}

#[test]
fn dto_conversion_preserves_statuses() {
    let api = HotSasApi::new(fake_services());
    let dto = api
        .get_product_workflow_status()
        .expect("must return status");

    for step in &dto.workflow_steps {
        assert!(
            ["ready", "limited", "unavailable", "not_configured", "error"]
                .contains(&step.status.as_str()),
            "step {} has invalid status: {}",
            step.id,
            step.status
        );
    }

    for module in &dto.module_statuses {
        assert!(
            ["ready", "limited", "unavailable", "not_configured", "error"]
                .contains(&module.status.as_str()),
...
#[test]
fn api_does_not_fail_when_ngspice_unavailable() {
    let api = HotSasApi::new(fake_services());
    let dto = api
        .get_product_workflow_status()
        .expect("must return status");
    ...
    assert!(
        sim_module.status == "ready" || sim_module.status == "limited",
        "simulation should not be error when ngspice unavailable"
    );
}
